...
namespace TrueWave
{
    ...
    public class LocalWavesPreset : ScriptableObject
    {
        ...
        [SerializeField]
        private SpectrumParams _spectrum = SpectrumParams.GetDefaultLocal();

        [TitleGroup("../Others", false)] [SerializeField]
        private float _referenceWaveHeight = 1;

        [SerializeField, Range(0, 2)] private float _chop = 1;

        [Tab("Tabs/Foam")] [MarkedUpField(false, false)] [SerializeField]
        private FoamParams _foam = FoamParams.GetDefault();

        [Tab("Tabs/Equalizer")] [InlineEditor] [SerializeField]
